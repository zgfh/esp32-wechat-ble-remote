/* Adapted from openscad-enclosure/assets/esp32_case_template.scad.
   PHOTO DRAFT: central black board + bare green remote PCB + future opto perfboard.
   Dimensions are UNMEASURED placeholders, not inferred physical measurements. */
$fn=48;
part="assembly"; // bottom, lid, layout, assembly
// UNMEASURED hardware envelopes INCLUDING underside joints. Confirm before printing.
esp_size=[55,30,12];
opto_size=[25,20,12];
remote_size=[45,30,10];
include_remote=true;
clearance=3;
module_gap=8;
wall=2.4;
...
cable_opening_width=26;
cable_opening_depth=19;
cable_opening_y=-38; // Align broad exit with ESP row; change after measuring connectors.
...
module bottom_case(){difference(){union(){
 ...
 for(side=[-1,1])translate([side*(outer_length/2+ear_projection/2-wall/2),20,0])rounded_box([ear_projection+wall,ear_width,ear_thickness],corner_radius);
 }
 screws()translate([0,0,max(floor_thickness,bottom_height-pilot_depth)])cylinder(h=pilot_depth+1,d=screw_pilot);
 for(side=[-1,1])translate([side*(outer_length/2+ear_projection/2),20,-1])cylinder(h=ear_thickness+2,d=ear_hole);
