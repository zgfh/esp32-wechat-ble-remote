// Synchronous, dependency-free SHA-256 for the WeChat Mini Program runtime.
module.exports = function (text) {
  const bytes = [];
  if (Array.isArray(text)) bytes.push(...text);
  else {
  ...
  return h.map(value => ('00000000' + value.toString(16)).slice(-8)).join('');
};
