...
<p align="center"><strong>微信小程序 → BLE → ESP32 → 光耦 → 原装遥控器按键</strong><br>用手机近距离模拟锁车与解锁短按。</p>
...
1. 在 `firmware/main.py` 的 `BLE_PAIRING_SECRET` 设置随机长字符串。
2. 在 `miniprogram/app.js` 的 `pairingSecret` 设置**相同**字符串；不要提交真实值。
3. 将 [`firmware/main.py`](firmware/main.py) 上传到 ESP32 MicroPython 根目录并运行；串口应显示 `BLE ready as ESP32-Car-Remote`。
4. 用微信开发者工具导入 [`miniprogram/`](miniprogram/)，执行“工具 → 构建 npm”安装 `crypto-js`。
...
| 认证 | `AUTH:SHA256(secret:nonce)` |
| 控制 | `CMD:LOCK`、`CMD:UNLOCK` |
