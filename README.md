...
> 想了解信号边界、后备箱放置方式和安全部署顺序？阅读 [工作原理与部署指南](docs/working-principle.md)。
...
可把仓库链接、ESP32 板型和遥控器按键照片交给可信 AI 编程代理，让它协助检查引脚、生成随机密码、上传文件、运行测试和解释串口日志。

请明确要求 AI：**不提交真实密码、不猜测遥控器焊盘、未获确认前不触发车辆控制**。详见 [部署指南](docs/working-principle.md#推荐让-ai-协助部署)。
